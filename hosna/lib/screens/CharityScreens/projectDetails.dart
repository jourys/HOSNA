...
//  checkVotingStatus() ;
  ...
  Future<void> checkIfDonorVoted() async {
    try {
      ...
      if (!projectSnapshot.exists) {
        print(
            "❌ Project document does not exist for projectId: ${widget.projectId}");
        return;
      }
      ...
      final VoteListener voteListener =
          VoteListener(projectId: widget.projectId);
      ...
      print(
          "🗳️ Calling hasDonorAlreadyVoted with votingId: $votingId and address: $ethAddress");
  ...
  Future<void> _fetchVotingStatus() async {
    ...
    try {
      ...
      print(
          '🧾 Checking if donor can vote using address: $globalWalletAddress');
      ...
      if (projectDoc.exists) {
        ...
        print(
            '❌ Project document not found in Firestore. Creating default document...');
        await projectDocRef.set({'votingInitiated': false});

        print(
            '✅ Default project document created with votingInitiated = false');
          ...
          if (isCompleted) {
            ...
            projectState = votingInitiated && (!isCompleted) && (!isEnded)
                ...
                : isCanceled && (!votingInitiated) && (!isEnded)
                    ? "canceled"
                    : getProjectState(data, votingInitiated, isCanceled,
                        isEnded, isCompleted);
          }
  ...
  void dispose() {
    _projectSubscription
        ?.cancel(); // Cancel the listener when the widget is removed
    super.dispose();
  }
...
_listenForSuspension(walletAddress.toString());
  ...
  String _getProjectState() {
    ...
    return getProjectState(
        project, votingInitiated, isCanceled, isEnded, isCompleted);
  }
...
// Set the global isCompleted value if it exists in Firestore
  ...
  String getProjectState(Map<String, dynamic> project, bool votingInitiated,
      bool isCanceled, bool isEnded, bool isCompleted) {
        ...
        children: [
          ...
          Container(
            padding: const EdgeInsets.symmetric(
                horizontal: 8, vertical: 6), // Adjust padding to fit the text
            decoration: BoxDecoration(
              color: Colors.grey.shade300, // Set the background color to grey
              borderRadius:
                  BorderRadius.circular(12), // Make the corners circular
            ),
            child: Text(
              value,
              style: TextStyle(
                fontSize: 16,
                color: Colors
                    .grey.shade700, // Text color inside the grey background
              ),
            ),
          )
        ],
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          Align(
                            alignment: Alignment
                                .centerLeft, // Aligns the widget to the left
                            child: _buildTypeItem(
                              '',
                              widget
                                  .projectType, // Passing the project type here
                            ),
                          ),
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment
                            .spaceBetween, // Space out the items
                        children: [
                          _buildDetailItem('Funded:',
                              '${widget.donatedAmount.toStringAsFixed(5)} ETH'),
                          _buildDetailItem(
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment
                            .spaceBetween, // Optional: Adjust alignment if needed
                        children: [
                          _buildDateItem(
                            'Begins:',
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if ((userType == 1 &&
                                    ...
                                    widget.projectCreatorWallet ==
                                        globalWalletAddress) ||
                                (userType == null && projectState == "active"))
                              Center(
                                child: ElevatedButton(
                                  onPressed: () async {
                                    ...
                                    bool confirmCancel = userType == null
                                        ? await _showcancelConfirmationDialogForAdmin(
                                            context)
                                    ...
                                    if (confirmCancel) {
                                      ...
                                      if (document.exists) {
                                        ...
                                        print(
                                            "Project state updated in Firestore.");
                                      } else {
                                        print(
                                            "Project document not found. Creating a new project...");
                                        await FirebaseFirestore.instance
                                            .collection('projects')
                                            .doc(widget.projectId.toString())
                                            .set({'isCanceled': true});
                                        print(
                                            "New project document created and canceled.");
                                      }
...
// Function to show confirmation dialog before cancellation
  Future<bool> _showcancelConfirmationDialogForAdmin(
    ...
    return await showDialog<bool>(
      ...
      if (confirmed == true && justificationController.text.trim().isNotEmpty) {
        ...
        _storeCancellationJustification(
            widget.projectId.toString(), justificationController.text.trim());
...
// Function to store cancellation justification and send notification to project creator
  ...
  Future<void> _storeCancellationJustification(
      String projectId, String justification) async {
    try {
...
// Function to send cancellation notification to project creator
  ...
  Future<void> _sendCancellationNotification(
    ...
    try {
      ...
      final body =
          'Your project "${widget.projectName}" has been cancelled by an admin. Reason: $justification';
  ...
  void _showDonationPopup(
      BuildContext context, double donatedAmount, double totalAmount) {
                    ...
                    decoration: InputDecoration(
                      ...
                      suffixIcon: Padding(
                        padding: const EdgeInsets.only(
                            right: 10, top: 10), // move ETH downward
                        child: Text(
                          'ETH',
                          style:
                    ...
                    onChanged: (value) {
                      setState(() {
                        ...
                        if (amount == null || amount <= 0) {
                          ...
                          errorMessage =
                              "Amount exceeds remaining goal of ${remaining.toStringAsFixed(5)} ETH";
                        } else {
                          errorMessage = null;
                        }
                      });
    ...
    try {
      ...
      final donationInfo = {
        ...
        'endDate': projectDetails['endDate'] is int
            ...
            : DateTime.parse(projectDetails['endDate'].toString())
...
// Check if donation already exists
      final existingIndex = donations.indexWhere(
          (d) => d['id'].toString() == projectDetails['id'].toString());
      if (existingIndex >= 0) {
        donations[existingIndex] = donationInfo;
        print(
            "✅ Updated existing donation for project ${projectDetails['name']}");
      } else {
...
// Fetch Firestore data for project status (e.g., canceled or voting initiated)
  Future<List<Map<String, dynamic>>> getEligibleVotingProjects(
    ...
    final allProjects = await blockchainService.fetchAllProjects();
    ...
    for (var project in allProjects) {
      ...
      if (!votingInitiated) continue;

      final canVote = await donorServices.checkIfDonorCanVote(
          BigInt.from(projectId), walletAddress);
      final votingId = int.tryParse((data?['votingId'] ?? '').toString()) ?? -1;

      final voteListener = VoteListener(projectId: projectId)
        ..initializeClient();
      final hasVoted = await voteListener.hasDonorAlreadyVoted(
          votingId, EthereumAddress.fromHex(walletAddress));

      final prefs = await SharedPreferences.getInstance();
      final privateKey = prefs.getString('privateKey_$walletAddress');
      ...
      if (canVote && (!hasVoted || !hasRequestedRefund)) {
        project['votingId'] = votingId;
                      ...
                      onPressed: () async {
                        try {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                                content:
                                    Text('Failed to submit complaint: $e')),
                          );
                        }
                      },
                      style: OutlinedButton.styleFrom(
